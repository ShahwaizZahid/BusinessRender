...
export default function BusinessSection() {
  return (
    <section className="flex w-full h-screen flex-col lg:flex-row">
      <div className="w-full  lg:w-1/2 bg-[#00102a] grid grid-cols-8 grid-rows-8 min-h-[300px] lg:min-h-full">
        {Array.from({ length: 64 }).map((_, i) => (
          <div key={i} className="border border-[#002c6f]" />
        ))}
      </div>
      <div className="w-full h-full lg:w-1/2  bg-[#002c6f]  px-8 py-16 md:p-20 flex flex-col justify-center items-center space-y-10">
